...
public class GraphPanel extends JPanel{
	...
	GraphPanel(Matrix points, int n){
		this.points = points;
		this.n = n;
		d = this.points.rows;
		pointsVec = new Vector[2];
		for(int i=0; i<2; i++) {
			pointsVec[i] = new Vector(d);
			for(int j=0; j<d; j++) {
				pointsVec[i].data[j] = this.points.data[j][i];
			}
		}
		
		approCoef = LeastSquare.minimize(pointsVec, n); // 近似関数の係数
//		for(int i=n; i>=0; i--) {
//			System.out.printf("+ %5.4f x^%d ", approCoef.data[i], i);
//		}

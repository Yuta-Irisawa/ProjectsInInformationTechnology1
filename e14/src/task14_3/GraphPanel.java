...
//			g2d.draw(new Line2D.Float(xi, yi, xi1, yi1));
	...
	float getXLower() {
		float xLower = (float)A.data[0][0];
		for(int i=1; i<getNumOfPoints(); i++) {
			if(xLower>(float)A.data[i][0]) {
				xLower = (float)A.data[i][0];
			}
		}
		return xLower;
	}
	...
	float getXUpper() {
		float xUpper = (float)A.data[0][0];
		for(int i=1; i<getNumOfPoints(); i++) {
			if(xUpper<(float)A.data[i][0]) {
				xUpper = (float)A.data[i][0];
			}
		}
		return xUpper;
	}
	...
	float getYLower() {
		float yLower = (float)A.data[0][1];
		
		for(int i=1; i<getNumOfPoints(); i++) {
			if(yLower>(float)A.data[i][1]) {
				yLower = (float)A.data[i][1];
			}
		}
		
//		for(int i=0; i<separateNum; i++) {
//			if(yLower>(float)approPoints[i].data[1]) {
//				yLower = (float)approPoints[i].data[1];
//			}
//		}
	...
	float getYUpper() {
		float yUpper = (float)A.data[0][1];
		
		for(int i=1; i<getNumOfPoints(); i++) {
			if(yUpper<(float)A.data[i][1]) {
				yUpper = (float)A.data[i][1];
			}
		}
		
//		for(int i=1; i<separateNum; i++) {
//			if(yUpper<(float)approPoints[i].data[1]) {
//				yUpper = (float)approPoints[i].data[1];
//			}
//		}
	...
	float getPointX(int i) {
		return (float)A.data[i][0];
	}
	
	float getPointY(int i) {
		return (float)A.data[i][1];
	}
